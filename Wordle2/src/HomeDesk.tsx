...
import type { HomeLayoutProps } from "./types/home";

const glassNav = (isDark: boolean) =>
  isDark
...
export default function HomeDesk({
  ...
  const onHintData = useCallback(
    (data: { hints?: string[]; availableHintCount?: number } | null) =>
      setHintData({
        hints: data?.hints ?? [],
        availableHintCount: data?.availableHintCount ?? 0,
      }),
    [],
  );
