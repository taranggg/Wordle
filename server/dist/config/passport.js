...
if (env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET) {
    passport.use(new GoogleStrategy({
        ...
        try {
            ...
            const baseUsername = email.replace(/@.*$/, "").replace(/[^a-zA-Z0-9_]/g, "_").slice(0, 20) || "user";
...
export function getGoogleAuthToken(user) {
    return jwt.sign({ id: String(user._id) }, env.JWT_SECRET, { expiresIn: "24h", algorithm: "HS256" });
}
